...
**Steps**:
1. [x] - `p2` - Extract permissions from SecurityContext bearer token via `toolkit-auth` - `inst-perm-1`
...
- **Performance (PERF)**: Not applicable for initial specification for the ancestor-chain merge walk (upstream < route < tenant hierarchy) — that walk is naturally bounded by how deep the requesting tenant itself is nested (typically ≤ 5 levels in practice, but not an enforced limit). Performance-critical caching of effective configuration belongs to Feature 8 (Observability & Security Hardening) multi-layer cache. A separate, *unbounded* traversal exists for budget validation (`domain/services/management/budget.rs`): both `validate_budget_allocation` and `validate_descendants_within_budget` call `get_descendants` with `GetDescendantsOptions::default()`, whose `max_depth: None` means unlimited — a single budget-affecting write walks the tenant's *entire* descendant subtree (not just its own ancestor chain), with no depth or result-size bound enforced by OAGW.
